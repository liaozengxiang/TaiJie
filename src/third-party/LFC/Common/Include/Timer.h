...
class CTimerEx;
struct ITimerEventEx
{
	virtual ~ITimerEventEx() {}
	virtual void OnTimer(CTimerEx *pTimer) = 0;
};

class CTimerEx : public CEvent
{
public:
	CTimerEx(CEventEngine *lpEngine, CWeakPtr<ITimerEventEx> wpTimerEventEx);		// 构造定时器对象，并指定定时事件的接收者
	virtual ~CTimerEx();
...
protected:
	Int32					m_nTimerFD;											// 定时器文件句柄
	CWeakPtr<ITimerEventEx>	m_wpTimerEvent;
};
#endif
